...
from two_cue_gridworld import (
    OBS_ROW, OBS_COL, OBS_LIGHT, OBS_ODOR,
)


def load_network(path="trained_agent.pt", obs_dim=4, n_actions=4):
...
def get_hidden(net, obs_batch):
    ...
    with torch.no_grad():
        h = net.trunk(obs_batch).numpy()  # (N, 64)
    return h


def make_obs(row, col, light, odor, env_size=11):
    """Single observation vector for given task variables."""
    o = np.zeros(4, dtype=np.float32)
    o[OBS_ROW]   = row / (env_size - 1)
    o[OBS_COL]   = col / (env_size - 1)
    o[OBS_LIGHT] = float(light)
    o[OBS_ODOR]  = float(odor)
    return o
