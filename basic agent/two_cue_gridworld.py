...
class TwoCueGridWorld:
    def __init__(self, size=5, max_steps=50, seed=None, sniff_bonus=5):
        ...
        center_col = size // 2
        self.start         = np.array([size // 2,  center_col])  # middle
        self.odor_port_0   = np.array([size // 2,  0])           # left
        self.odor_port_1   = np.array([size // 2,  size - 1])    # right
        self.reward_port_0 = np.array([0,          0])           # top-left
        self.reward_port_1 = np.array([0,          size - 1])    # top-right
